"""
Simulation visualization: merge multiple OBJ frames (sharing identical
topology) into a single glTF-Binary (.glb) file with morph-target animation.
Used by both thread (single animation) and patch (six per-experiment animations).
...
def build_morph_target_glb(obj_texts: list[str]) -> bytes:
    ...
    logger.info(f"[sim-viz] parsing {len(obj_texts)} OBJ frames")
    ...
    for i, text in enumerate(obj_texts):
        ...
        logger.info(f"[sim-viz] frame {i}: {len(frames[-1].positions)} verts, "
    ...
    logger.info(f"[sim-viz] glTF vertex count: {V}, triangles: {len(indices)//3}, "
    ...
    gltf = {
        'asset': {'version': '2.0', 'generator': 'HESTIA simulation-visualization'},
        'scene': 0,
        'scenes': [{'nodes': [0]}],
        'nodes': [{'mesh': 0}],
        'meshes': [{
    ...
    logger.info(f"[sim-viz] built GLB: {total_len} bytes ({len(morph_targets)} morph targets)")
    return bytes(out)